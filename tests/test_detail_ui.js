...
assert(index.includes('src="detail-ui.js?v=41"'));
assert(app.includes('查看官方原始公告'), "announcement cards expose the official source");
assert(app.includes('window.open(item.url, "_blank", "noopener,noreferrer")'), "legacy source buttons open the official source");
assert(!app.includes('fetch(item.detail_ref'), "public app must not fetch static full-text sidecars");
assert(app.includes('return Promise.resolve(null);'), "assistant must not load full-text sidecars");
