...
from timetable import find_current_timetable_article, find_pdf_attachment, is_direct_pdf_url  # noqa: E402
...
def run():
    ...
    assert article == {
        "url": "https://www.cysh.cy.edu.tw/formal",
        "label": "115-1正式課表",
        "version": "formal",
    }

    attachment = find_pdf_attachment(
        '<a href="/teacher.pdf">115-1教師課表.pdf</a>'
        '<a href="/app/index.php?Action=downloadfile&file=x">115-1班級大課表.pdf</a>',
        detail_url=article["url"],
    )
    assert attachment["url"].startswith("https://www.cysh.cy.edu.tw/app/index.php?")

    news = """<a href="/var/file/8/1008/img/6/teacher.pdf">115-1正式教師課表(另開新視窗.pdf)</a>
      <a href="/var/file/8/1008/img/6/384591916.pdf">115-1正式班級課表(另開新視窗_pdf)</a>"""
    news_article = find_current_timetable_article(
        news,
        academic_year=115,
        semester=1,
        base_url="https://www.cysh.cy.edu.tw/p/403-1008-12-1.php",
    )
    assert news_article == {
        "url": "https://www.cysh.cy.edu.tw/var/file/8/1008/img/6/384591916.pdf",
        "label": "115-1正式班級課表(另開新視窗_pdf)",
        "version": "formal",
    }
    assert is_direct_pdf_url(news_article["url"])
    assert not is_direct_pdf_url("https://evil.example/formal.pdf")
