...
const client = {
  ...
  from(table) {
    return {
      ...
      upsert(rows) { calls.push({ table, rows }); return Promise.resolve({ data: rows, error: null }); },
    };
  },
};
...
adapter.fetchRemoteState().then(async remote => {
  ...
  await adapter.pushRows("user_subscriptions", [{ user_id: "user-b", keyword: "X", createdAt: "2026-01-01T00:00:00Z" }]);
