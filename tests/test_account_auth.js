...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="app.js?v=75"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v76"), "Service Worker cache must advance for the current app shell");
  assert(app.includes('register("sw.js?v=75")'), "App must register the current Service Worker script");
