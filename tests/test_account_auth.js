...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="app.js?v=91"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v95"), "Service Worker cache must advance for the current app shell");
