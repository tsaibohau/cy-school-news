...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="account-sync.js?v=35"'), "index must load versioned Account Sync before app.js");
  assert(index.includes('src="account-config.js?v=35"'), "index must load versioned account config");
  assert(index.includes('src="account-auth.js?v=35"'), "index must load versioned account auth");
  assert(sw.includes('"./account-sync.js?v=35"'), "Service Worker shell must cache versioned Account Sync");
  ...
  assert(sw.includes("cy-news-v36"), "Service Worker cache must advance for the current app shell");
  assert(app.includes('register("sw.js?v=35")'), "App and Service Worker must use one shell version");
