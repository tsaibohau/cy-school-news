...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="app.js?v=64"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v65"), "Service Worker cache must advance for the current app shell");
  assert(app.includes('register("sw.js?v=65")'), "App and Service Worker must use one shell version");
  ...
  assert(index.includes('id="adminPanel"'));
