...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="account-auth.js?v=45"'), "index must load current account auth");
  assert(index.includes('src="app.js?v=62"'), "index must load current app shell");
  assert(sw.includes('"./account-sync.js?v=54"'), "Service Worker shell must cache versioned Account Sync");
  assert(sw.includes('"./account-auth.js?v=45"'), "Service Worker shell must cache current account auth");
  ...
  assert(sw.includes("cy-news-v63"), "Service Worker cache must advance for the current app shell");
  assert(app.includes('register("sw.js?v=63")'), "App and Service Worker must use one shell version");
