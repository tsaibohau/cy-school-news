...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert.equal(Auth.validPassword("123456"), true);
  assert.equal(Auth.validPassword("a".repeat(73)), false);
  await controller.signUpWithPassword("test@example.test", "abc123", "", "test_new");
  assert.equal(passwordSignUpRequest.password, "abc123");
  await controller.updatePassword("abc123");
  assert.equal(userUpdateRequest.password, "abc123");
  ...
  assert.match(Auth.signUpErrorMessage({ code: "email_address_not_authorized" }), /驗證信服務/);
  ...
  assert(index.includes('src="account-config.js?v=44"'), "index must load versioned account config");
  assert(index.includes('src="account-auth.js?v=79"'), "index must load current account auth");
  assert(index.includes('src="app.js?v=94"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v99"), "Service Worker cache must advance for the current app shell");
  ...
  assert(index.includes('id="passwordAuthUsernameField"'));
  assert(index.includes('id="passwordAuthPasswordField" hidden'));
  assert(index.includes('id="passwordAuthChangeIdentifier"'));
  assert(index.includes('沒有帳號？'));
  assert(app.includes('setPasswordSignInStep("password")'));
  assert(index.includes("Email 或帳號名稱"));
  assert(index.includes('id="viewAdmin"'));
  assert(index.includes('id="tabAdmin"'));
  assert(app.includes("getAdminAccounts"));
  assert(app.includes("updateAccountAccess"));
  assert(app.includes("setAdminRole"));
  assert(app.includes("requestAccountAccess"));
  assert(app.includes("getAccountAccess"));
  assert(app.includes("等待管理員核准期間，仍可使用目前開放的公開功能"));
