...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  const appRevision = index.match(/src="app\.js\?v=(\d+)"/);
  const cacheRevision = sw.match(/var CACHE = "cy-news-v(\d+)"/);
  assert.ok(appRevision && cacheRevision, "current app and PWA shell revisions are declared");
  assert(sw.includes('"./app.js?v=' + appRevision[1] + '"'), "Service Worker caches the active app shell");
  ...
  assert(app.includes('register("sw.js?v=' + cacheRevision[1] + '")'), "App and Service Worker must use one shell version");
