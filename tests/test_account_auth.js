...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="app.js?v=89"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v93"), "Service Worker cache must advance for the current app shell");
