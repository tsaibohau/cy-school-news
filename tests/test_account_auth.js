...
singletonAndRetryChecks().then(() => controller.signInWithGoogle()).then(async () => {
  ...
  assert(index.includes('src="app.js?v=90"'), "index must load current app shell");
  ...
  assert(sw.includes("cy-news-v94"), "Service Worker cache must advance for the current app shell");
