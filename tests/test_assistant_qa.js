...
const expiredDorm = { id: "expired-dorm", title: "宿舍申請公告", date: "2026-08-01", summary: "申請期間為115年8月1日至8月3日止。" };
const expiredAnswer = QA.answer("現在可以申請宿舍嗎", [expiredDorm], {}, { asOf: "2026-08-30" });
assert.equal(expiredAnswer.status, "answered");
assert.match(expiredAnswer.summary, /期限/);
assert.match(expiredAnswer.answer_lines[0], /已於 2026-08-03/);
assert.equal(expiredAnswer.sources[0].validity.status, "EXPIRED");
const oldStandingRule = { id: "old-rule", title: "學生宿舍管理規定", date: "2022-08-01", summary: "住宿學生應遵守門禁規定。" };
const standingAnswer = QA.answer("現在宿舍門禁有什麼規定", [oldStandingRule], {}, { asOf: "2026-08-30" });
assert.equal(standingAnswer.status, "answered", "publication age cannot suppress a standing rule");
assert.notEqual(standingAnswer.sources[0].validity.status, "EXPIRED");
const mixedOldRegistration = { id: "old-contest", title: "競賽報名及規則", summary: "報名至2026年8月20日止。競賽外殼限制請參閱附件。" };
const currentRegistration = QA.answer("現在有哪些競賽可以報名", [mixedOldRegistration], {}, { asOf: "2026-08-30" });
assert.match(currentRegistration.summary, /超過明確期限/, "an expired window cannot appear as currently open merely because its rules remain useful");
