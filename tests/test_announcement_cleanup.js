...
assert.equal(byId.deadline.reference_value, "LOW");
assert.equal(byId.event.reason, "event_ended");
assert.equal(byId.term.reason, "term_ended");
assert.equal(byId.old.reason, "replaced");
assert.equal(byId.old.reference_value, "HIGH");
assert.equal(byId.old.superseded_by, "new");
assert.deepEqual(byId.term.academic, { academic_year: 114, semester: 2 });
...
assert(!cleanup.scan(rows, [{ announcement_id: "deadline", action: "archive" }], { today: "2026-09-11" }).some((row) => row.announcement_id === "deadline"));
