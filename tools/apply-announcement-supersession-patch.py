...
helper_marker = '  function rank(query, items, details, options) {'
  ...
  function preferNewestRelated(query, ranked) {
    ...
    chronological.forEach(function (row) {
      var superseded = kept.some(function (newer) {
        ...
        /* Field-scoped replacement: a newer notice only replaces the older
           answer for the field that the newer notice actually addresses. */
...
rank_floor = '    var floor = Math.max(70, ranked[0].score * 0.55);\n    return ranked.filter(function (row) { return row.score >= floor; });'
if 'ranked = preferNewestRelated(query, ranked);' not in qa:
    replacement = '    ranked = preferNewestRelated(query, ranked);\n    if (!ranked.length) return ranked;\n' + rank_floor
    if rank_floor not in qa:
        raise SystemExit('rank floor marker not found')
    qa = qa.replace(rank_floor, replacement, 1)

export_marker = '  return { tokens: tokens, anchors: anchors, literalAnchors:literalAnchors, intent: intent, questionPlan: questionPlan, detailText: detailText, rank: rank,'
if 'preferNewestRelated:preferNewestRelated' not in qa:
    replacement = '  return { tokens: tokens, anchors: anchors, literalAnchors:literalAnchors, intent: intent, questionPlan: questionPlan, detailText: detailText, rank: rank, preferNewestRelated:preferNewestRelated,'
    if export_marker not in qa:
        raise SystemExit('export marker not found')
    qa = qa.replace(export_marker, replacement, 1)

qa_path.write_text(qa, encoding='utf-8')

test = test_path.read_text(encoding='utf-8')
if 'newer same-topic notice must replace the old location field' not in test:
    insert_before = 'const lowConfidenceDetail = {'
...
'''
    ...
    test = test.replace(insert_before, block + insert_before, 1)
