...
def extract_pdf_text(content):
    ...
    for page in reader.pages:
        # Keep the extractor's native line/reading-order shape.  The school
        # adapters below rebuild table rows; they do not assume one line/event.
...
def _calendar_year(academic_year, semester, month):
    """Resolve M/D against a Taiwan school term, including semester-1 January."""
...
def _cysh_week_anchors(lines, academic_year, semester):
    ...
    finish = start + timedelta(days=220)
    candidates = []
    cursor = start
    while cursor <= finish:
    ...
    for index, line in enumerate(lines):
        days = [int(value) for value in re.findall(r"(?<!\d)(\d{1,2})(?!\d)", line[:100])]
        if len(days) < 7:
            continue
        days = days[:7]
    ...
    for line_index, line in enumerate(lines):
        ...
        for match_index, match in enumerate(matches):
            ...
            if len(title) < 2 or title in {"V1", "星期", "None"}:
                continue
            start_options = [day for day in week if day.day == int(match["d1"])]
...
def _parse_cygsh_layout(text, **context):
    """Rebuild numbered work rows by department; use only the first row date."""
    rows = []
    items, current = [], None
    for raw in str(text).splitlines():
        ...
        if numbered:
            ...
            # Native extraction keeps each numbered cell contiguous.  A new
            # numbered item closes the row; wrapped prose is continuation only.
            current += " " + line
    if current:
        items.append(current)
    for item in items:
        title = _normal_title(item)
        # One numbered table row creates at most one event.  Later dates,
        # times and lesson periods stay inside its title and never fan out.
        stamp = RANGE.search(title) or MD.search(title)
        if not stamp:
            continue
        if stamp.re is RANGE:
            month, day = int(stamp["m1"]), int(stamp["d1"])
        else:
            month, day = int(stamp["month"]), int(stamp["day"])
        try:
...
def calendar_quality_gate(events, *, school_id, academic_year, semester,
    ...
    return {
        "passed": not reasons, "event_count": len(rows), "months": months,
