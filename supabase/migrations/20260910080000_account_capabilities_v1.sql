...
create or replace function public.has_account_capability(p_capability text) returns boolean language sql stable security definer set search_path=pg_catalog,public as $$
...
$$;
revoke all on function public.has_account_capability(text) from public,anon;
grant execute on function public.has_account_capability(text) to authenticated;

create or replace function public.current_account_capabilities() returns table(capability text,enabled boolean) language sql stable security definer set search_path=pg_catalog,public as $$
...
$$;
revoke all on function public.current_account_capabilities() from public,anon;
grant execute on function public.current_account_capabilities() to authenticated;

create or replace function public.admin_set_account_capabilities(target_user_id uuid,next_capabilities jsonb) returns void language plpgsql security definer set search_path=pg_catalog,public as $$
...
begin
 ...
 if exists(select 1 from public.app_admins where user_id=target_user_id and revoked_at is null and admin_role in ('owner','co_admin')) then raise exception 'admin_capabilities_protected' using errcode='42501'; end if;
...
revoke all on function public.admin_set_account_capabilities(uuid,jsonb) from public,anon;
