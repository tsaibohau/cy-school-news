...
function reply(body: Record<string, unknown>, status: number, origin: string | null): Response {
  const allowedOrigin = origin && allowedOrigins.has(origin) ? origin : "https://cy-school-news-staging.vercel.app"
...
Deno.serve(async (req) => {
  ...
  if (req.method === "OPTIONS" && origin && allowedOrigins.has(origin)) return reply({ ok: true }, 200, origin)
  if (req.method !== "POST" || !origin || !allowedOrigins.has(origin)) return reply({ error: "not_found" }, 404, origin)
  ...
  if (lookupError || typeof email !== "string") return reply({ error: "invalid_credentials" }, 400, origin)
  const authResponse = await fetch(`${url}/auth/v1/token?grant_type=password`, {
    method: "POST",
    headers: { apikey: publishableKey(), "content-type": "application/json" },
    body: JSON.stringify({ email, password }), signal: AbortSignal.timeout(10000),
  })
  if (!authResponse.ok) return reply({ error: "invalid_credentials" }, 400, origin)
  const tokens = await authResponse.json() as { access_token?: string; refresh_token?: string; expires_in?: number; token_type?: string }
  if (!tokens.access_token || !tokens.refresh_token) return reply({ error: "invalid_credentials" }, 400, origin)
  return reply({ access_token: tokens.access_token, refresh_token: tokens.refresh_token, expires_in: tokens.expires_in, token_type: tokens.token_type }, 200, origin)
})
