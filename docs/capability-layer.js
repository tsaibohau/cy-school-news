...
(function () {
  ...
  function normalize(rows) {
    ...
    (Array.isArray(rows) ? rows : []).forEach(function (row) { if (row && KEYS.indexOf(row.capability) !== -1) out[row.capability] = row.enabled === true; });
    return out;
  }
  ...
  function applyNavigation() {
    ...
    Object.keys(map).forEach(function (id) { var node = document.getElementById(id); if (node && approved) node.hidden = !allowed(map[id]); });
    ["tabHome", "tabToday"].forEach(function (id) { var node = document.getElementById(id); if (node && approved) node.hidden = !(allowed("assistant") || allowed("timetable") || allowed("calendar") || allowed("notifications")); });
  }
  function renderAdminCapabilities() {
    ...
    root.querySelectorAll(".admin-account").forEach(function (card) {
      ...
      var userNode = card.querySelector("[data-admin-user]");
      ...
      box.innerHTML = "<legend>功能權限</legend>" + KEYS.map(function (key) {
        return '<label><input type="checkbox" data-capability="' + key + '"' + (adminRows[userId][key] ? " checked" : "") + '> ' + LABELS[key] + "</label>";
      }).join("") + '<span class="capability-save-state" aria-live="polite"></span>';
      box.addEventListener("change", function (event) {
        if (!event.target.matches("input[data-capability]")) return;
        var next = {};
        KEYS.forEach(function (key) { var input = box.querySelector('input[data-capability="' + key + '"]'); next[key] = !!(input && input.checked); });
    ...
    set: function (api) {
      ...
      api.createController = function (options) {
        ...
        controller.getAdminAccounts = function (filters) {
          return originalAdmin.call(controller, filters).then(function (rows) {
            ...
            return controller.getClient().then(function (c) { return c.rpc("admin_account_capabilities", { target_user_ids: ids }); }).then(function (result) {
              ...
              (result.data || []).forEach(function (row) { if (adminRows[row.user_id] && KEYS.indexOf(row.capability) !== -1) adminRows[row.user_id][row.capability] = row.enabled === true; });
        ...
        controller.setAccountCapabilities = function (userId, capabilities) {
          return controller.getClient().then(function (c) { return c.rpc("admin_set_account_capabilities", { target_user_id: userId, next_capabilities: capabilities }); }).then(function (result) { if (result.error) throw result.error; });
        };
