...
- 公告清單依校網第一筆資料回報的公告總數逐頁讀完（每頁 30 則），頁與頁之間至少暫停 1.5 秒。安全上限為 100 頁／3,000 則；若校網回報更多、提前結束或最後數量不一致，整次工作明確失敗，不把不完整資料當成成功。公開資料整理與測試站覆蓋層使用相同 3,000 則上限，不能在抓取完成後再截斷。
- 會員摘要每次最多讀取最新 5 篇公告詳情，各篇之間至少暫停 1.5 秒。所有請求都依序進行，不做平行大量存取。
- 附件摘要只允許官方同網域、大小受限且具有文字層的 PDF；不略過憑證、不保存或公開附件副本，也不對掃描圖片假稱已有摘要。Word 與掃描型 PDF 尚未啟用。
- Windows 完成抓取後的資料提交必須觸發 Vercel 重新建置，否則分支雖已有新公告，測試站仍會顯示抓取前的舊資料。該資料提交會略過第二次 Windows 抓取，避免重複請求校網。
