...
### 最終狀態
【已完成】
- 2026-09-25 發行準備 checkpoint：從最新 `origin/main` `7ca0911` 建立 `codex/pr30-pages-release-20260925`，只取 PR #30 的前端、三個 migration、測試及 staging build；保留 `main` 所有 `docs/data/*` 與爬蟲輸出，272 筆 PDF 候選僅供 staging build。PR #30 分支 `29b1de4` 已遠端保存；未併入 `main`。
- Production 遷移前 SELECT 基準（帳號依 UUID 排序匿名編號）：1 approved/none/5 項中 1 啟用（assistant）；2 approved/co_admin/5 中 5；3 approved/none/5 中 5；4 approved/owner/5 中 5。正式執行前須重新讀取並在遷移後逐帳號比對。三個 migration 不直接更新既有 `account_capabilities`；新 `owner_set_admin_role` 被日後調用且授予 owner/co_admin 時會更新對象 `service_level` 為 full。
- Production 實際 schema 與 Preview 抽查：正式庫缺 `user_calendar_events`、`public_capabilities`，保有 `app_admins_one_active_owner`；正式庫三個既有函式會被重新定義，兩個公告函式增加 anon EXECUTE 與受 PUBLIC 開關約束的匿名讀取。Preview 有目標結構；本輪 Preview SQL 再查遭 MCP permission denied，未宣稱完成整庫等價。GitHub Actions PR #30 user_tasks matrix 6/25 FAIL，calendar 27/27、PUBLIC 15/15 PASS；需要先釐清 baseline 才能放行。
- 本地發行檢查：曾發現 PR #30 測試版號仍檢查 app v87／SW v91，而前端為 v88／v92，已在發行候選更正測試；browser load order、auth、account sync、calendar persistence、staging build、calendar workflow、UI 與 PWA 相關檢查 PASS。`python tests/test_parser.py` 因本地缺 `requests` 無法執行，爬蟲邏輯本次沒有改動。正式站／Production DB 均未改；尚未完成正式回復演練、Production 遷移或發布。
- 2026-09-25 雲端 checkpoint：發行候選 `2575eb0` 已由 Codespace 原生 git push 到 `codex/pr30-pages-release-20260925`，遠端 parity PASS；Draft PR #31 https://github.com/tsaibohau/cy-school-news/pull/31 可合併但狀態 UNSTABLE，Vercel Preview Ready（`https://cy-school-news-staging-git-code-35f5f3-tsaibohau-9644s-projects.vercel.app`）。GitHub Pages API 確認正式來源為 `main` 的 `/docs`，build_type legacy。
